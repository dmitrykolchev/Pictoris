...
public static class XMath
{
    ...
    private static readonly UXMVECTOR _select1000 = Vector128.Create<uint>([0xFFFFFFFFu, 0, 0, 0]);
    private static readonly UXMVECTOR _select1100 = Vector128.Create<uint>([0xFFFFFFFFu, 0xFFFFFFFFu, 0, 0]);
    private static readonly UXMVECTOR _select1110 = Vector128.Create<uint>([0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu, 0]);
    private static readonly UXMVECTOR _select1011 = Vector128.Create<uint>([0xFFFFFFFFu, 0, 0xFFFFFFFFu, 0xFFFFFFFFu]);
    ...
    private static UXMVECTOR XMVectorLess(FXMVECTOR v1, FXMVECTOR v2)
    {
        if (Sse.IsSupported)
        {
            return Sse.CompareLessThan(v1, v2).AsUInt32();
        }
        return Vector128.Create(
    ...
    private static UXMVECTOR XMVectorGreater(FXMVECTOR v1, FXMVECTOR v2)
    {
        if (Sse.IsSupported)
        {
            return Sse.CompareGreaterThan(v1, v2).AsUInt32();
        }
        return Vector128.Create(
    ...
    private static unsafe FXMVECTOR XMVectorSelect(FXMVECTOR v1, FXMVECTOR v2, UXMVECTOR control)
    {
        if (Sse.IsSupported)
        {
            var c = control.AsSingle();
            var temp1 = Sse.AndNot(c, v1);
            var temp2 = Sse.And(v2, c);
            return Sse.Or(temp1, temp2);
        }
        var u1 = v1.AsUInt32();
        var u2 = v2.AsUInt32();
        var result = Vector128.Create(
            (u1[0] & ~control[0]) | (u2[0] & control[0]),
            (u1[1] & ~control[1]) | (u2[1] & control[1]),
            (u1[2] & ~control[2]) | (u2[2] & control[2]),
            (u1[3] & ~control[3]) | (u2[3] & control[3]));
